...
const PROXY_URL =
  'postgresql://customer_service_app@revenda-prod-customer-proxy.proxy-abc.us-east-1.rds.amazonaws.com:5432/customers?schema=public&sslmode=require';
...
describe('conexão ao banco por token IAM', () => {
  it('extrai host, porta, usuário e banco da DATABASE_URL e exige TLS', () => {
    ...
    expect(config).toMatchObject({
      host: 'revenda-prod-customer-proxy.proxy-abc.us-east-1.rds.amazonaws.com',
      port: 5432,
  ...
  it('pede um token novo a cada conexão aberta', async () => {
    ...
    expect(requests[0]).toEqual({
      hostname: 'revenda-prod-customer-proxy.proxy-abc.us-east-1.rds.amazonaws.com',
      port: 5432,
  ...
  it('recusa DATABASE_URL com senha embutida', () => {
    expect(() =>
      buildIamPoolConfig('postgresql://app:senha@proxy.local:5432/customers', 'us-east-1'),
