...
const PROXY_URL =
  'postgresql://vehicle_service_app@revenda-prod-vehicle-proxy.proxy-abc.us-east-1.rds.amazonaws.com:5432/vehicles?schema=public&sslmode=require';
...
describe('conexão ao banco por token IAM', () => {
  it('extrai host, porta, usuário e banco da DATABASE_URL e exige TLS', () => {
    ...
    expect(config).toMatchObject({
      host: 'revenda-prod-vehicle-proxy.proxy-abc.us-east-1.rds.amazonaws.com',
      port: 5432,
      user: 'vehicle_service_app',
      database: 'vehicles',
  ...
  it('pede um token novo a cada conexão aberta', async () => {
    ...
    expect(requests[0]).toEqual({
      hostname: 'revenda-prod-vehicle-proxy.proxy-abc.us-east-1.rds.amazonaws.com',
      port: 5432,
      username: 'vehicle_service_app',
      region: 'us-east-1',
    });
  });
  ...
  it('recusa DATABASE_URL com senha embutida', () => {
    expect(() =>
      buildIamPoolConfig('postgresql://app:senha@proxy.local:5432/vehicles', 'us-east-1'),
