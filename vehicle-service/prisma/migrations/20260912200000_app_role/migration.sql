...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'vehicle_service_app') THEN
    CREATE ROLE vehicle_service_app LOGIN;
  END IF;
END
$$;

GRANT USAGE ON SCHEMA public TO vehicle_service_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON "vehicles", "outbox_events" TO vehicle_service_app;

-- Nenhuma permissão de DDL: a aplicação não altera o schema em tempo de execução.
REVOKE CREATE ON SCHEMA public FROM vehicle_service_app;
